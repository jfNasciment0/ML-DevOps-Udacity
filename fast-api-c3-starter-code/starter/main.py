...
app = FastAPI(title="Inference API", description="An API that takes a sample and runs an inference", version="1.0.0")
...
async def lifespan(app: FastAPI):
    ...
    if os.path.isfile(os.path.join(model_dir, filename[0])):
        model = pickle.load(open(os.path.join(model_dir, filename[0]), "rb"))
        encoder = pickle.load(open(os.path.join(model_dir, filename[1]), "rb"))
        lb = pickle.load(open(os.path.join(model_dir, filename[2]), "rb"))
...
async def ingest_data(inference: InputData):
    ...
    sample, _, _, _ = process_data(sample, categorical_features=cat_features, training=False, encoder=encoder, lb=lb)
